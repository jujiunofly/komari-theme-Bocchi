...
const visitorStatusText = computed(() => {
  ...
  if (visitorFailed.value)
    return '定位还没回来，先显示这台设备'
  return `来自 ${welcomeLocation.value} 的观众已进线`
})
...
<template>
  ...
  <Transition name="slide-left">
    <div
      v-if="show && !dismissed"
      class="visitor-info-card fixed bottom-16 left-3 z-50 hidden w-56 max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-2xl 2xl:block
             backdrop-blur-xl
             shadow-2xl"
    >
      <!-- 顶部：头像 + 名字 + 关闭 -->
...
.visitor-info-pill {
  ...
  background: linear-gradient(110deg, rgb(255 255 255 / 0.72), rgb(236 250 255 / 0.62), rgb(250 234 255 / 0.64));
  box-shadow:
    0 10px 32px rgb(91 33 182 / 0.16),
...
.visitor-info-card {
  ...
  background:
    radial-gradient(circle at 18% 0%, rgb(34 211 238 / 0.17), transparent 36%),
    radial-gradient(circle at 94% 18%, rgb(167 139 250 / 0.2), transparent 38%),
    radial-gradient(circle at 70% 100%, rgb(244 114 182 / 0.13), transparent 42%),
    linear-gradient(145deg, rgb(255 255 255 / 0.68), rgb(245 238 255 / 0.58));
  box-shadow:
    0 18px 48px rgb(91 33 182 / 0.18),
    inset 0 1px rgb(255 255 255 / 0.92);
}
