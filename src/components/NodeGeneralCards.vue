...
const wrapperClass = computed(() => {
  ...
  return 'gloria-overview p-4 grid grid-cols-12 gap-3 h-auto md:min-h-[17.25rem]'
})
const earthClass = computed(() => {
  return 'col-span-12 row-start-1 md:col-span-6 md:col-start-7 md:row-start-1'
})
const cardGridClass = computed(() => {
  if (!showEarth.value)
    return 'col-span-1 grid grid-cols-3 md:grid-cols-6 gap-2'
  return 'col-span-12 row-start-2 z-9 md:col-span-6 md:row-start-1 grid grid-cols-12 auto-rows-[5.4rem] md:auto-rows-[8rem] gap-2'
})
const cardClass = 'gloria-stat-card group relative z-10 h-full bg-background/50 border-none hover:bg-background backdrop-blur-sm md:backdrop-blur-none transition-all'
  ...
  <div v-if="shouldRenderHeader" :class="wrapperClass">
    ...
    <div v-if="visibleCards.length > 0" :class="cardGridClass">
      <CardX
        ...
        :aria-label="card.action ? `查看${card.key === 'remainingValue' ? '剩余价值' : card.label}明细` : undefined"
