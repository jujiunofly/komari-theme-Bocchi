...
const LEGACY_GLORIA_GENERAL_CARD_ORDER: GeneralCardKey[] = [
  'onlineNodes',
  'avgCpu',
  'memory',
  'disk',
...
const LEGACY_BASE_GENERAL_CARD_ORDER: GeneralCardKey[] = [
  'memory',
  'disk',
  'remainingValue',
  'totalTraffic',
...
const DEFAULT_GLASS_CUSTOM_COLORS: GlassCustomColors = {
  lightCard: '#ffffffa8',
  lightControl: '#ffffffb8',
  lightText: '#3b2433',
  lightMutedText: '#7a4d62',
  lightBorder: '#ffd0e4cc',
  darkCard: '#140812cc',
  darkControl: '#140812d9',
  darkText: '#fff1f7',
  darkMutedText: '#f3c3d6',
  darkBorder: '#ff4fa340',
}

const HEX_COLOR_REGEX = /^#[0-9a-f]{6}(?:[0-9a-f]{2})?$/i
const KEY_LIST_SEPARATOR_REGEX = /[\s,，;；]+/u
const AUDIO_SOURCE_REGEX = /^(?:https?:\/\/|\/)/i
const REGION_AUDIO_CODE_REGEX = /^[A-Z0-9-]{2,8}$/
...
function resolveAudioSource(value: string): string {
  const resolved = resolveBackgroundSource(value)
  return AUDIO_SOURCE_REGEX.test(resolved) ? resolved : ''
}
...
const useAppStore = defineStore('app', () => {
  ...
  const fanLabelsEnabled = computed<boolean>(() => readBooleanSetting(themeSettings.value, 'fanLabelsEnabled', true))

  const fanAudioPreviewEnabled = computed<boolean>(() => readBooleanSetting(themeSettings.value, 'fanAudioPreviewEnabled', false))

  const fanAudioPreviewVolume = computed<number>(() => readNumberSetting(themeSettings.value, 'fanAudioPreviewVolume', 35, 0, 100))

  const fanAudioPreviewDuration = computed<number>(() => readNumberSetting(themeSettings.value, 'fanAudioPreviewDuration', 25, 20, 30))

  const fanAudioTrackSources = computed<Record<string, string>>(() => {
    const configured = readStringRecord(themeSettings.value.fanAudioTrackSources)
    const sources: Record<string, string> = {}
    for (const [track, value] of Object.entries(configured)) {
  ...
  const regionAudioTracks = computed<Record<string, string>>(() => {
    const configured = readStringRecord(themeSettings.value.regionAudioTracks)
    const tracks: Record<string, string> = {}
    for (const [region, track] of Object.entries(configured)) {
      const code = region.toUpperCase()
      if (REGION_AUDIO_CODE_REGEX.test(code))
        tracks[code] = track
    }
    return tracks
  })

  const gloriaHeroType = computed<'image' | 'video'>(() => {
    return themeSettings.value.gloriaHeroType === 'video' ? 'video' : 'image'
  })

  const gloriaHeroUrl = computed<string>(() => {
  ...
  const generalCardOrder = computed<GeneralCardKey[]>(() => {
    ...
    if (hasNewPreset) {
      if (preset === 'custom') {
        const advancedKeys = typeof settings.generalCardKeys === 'string'
          ...
          : ''
        if (advancedKeys)
          return migrateGloriaGeneralCardOrder(parseKeyList(advancedKeys, isGeneralCardKey, DEFAULT_GENERAL_CARD_ORDER))
    ...
    if (typeof settings.generalCardKeys === 'string')
      return migrateGloriaGeneralCardOrder(parseKeyList(settings.generalCardKeys, isGeneralCardKey, DEFAULT_GENERAL_CARD_ORDER))
  ...
  return {
    ...
    hideGeneralCard,
    fanLabelsEnabled,
    fanAudioPreviewEnabled,
    fanAudioPreviewVolume,
    fanAudioPreviewDuration,
    fanAudioTrackSources,
    regionAudioTracks,
    gloriaHeroType,
    gloriaHeroUrl,
    easterEyebrow,
